...
for(i in seq_len(nrow(wgtlist))) {
  ...
  # heritability filter: skip if hsq.pv is missing or > threshold
  model_hsq    <- env$hsq[1]
  hsq <- model_hsq
...
# 5) Bonferroni‐correct & write only the significant subset
tested <- !is.na(out$TWAS.P)
N_eff  <- sum(tested)
Pcrit  <- 0.05 / N_eff
significant <- subset(out, tested & TWAS.P < Pcrit)

sign_file <- sub("\\.dat$", ".significant.dat", opt$out)
write.table(significant, sign_file,
            sep = "\t", quote = FALSE, row.names = FALSE)

cat("Number of test =", N_eff, "\n")
